...
6. SCROLL TO CONTENT
7. CONTENT APPEAR ANIMATION INIT / #ksphere-details
8. CONTENT CLOSE INITI / #ksphere-close-content
9. NEWSLETTER FORM INIT / #ksphere-form
...
21. Preloader / .ksphere-loading-screen
...
function contentToggle($this){
  ...
  var self    = $this,
    details   = $('#ksphere-details'),
    content   = $('#ksphere-animate-area'),
    close     = $('#ksphere-close-content'),
    hide      = content.data('hide'),
    animation = details.data('animation');

  if ( self.hasClass('ksphere-animate-open') ) {
    close.hide(0);
    self.removeClass('ksphere-animate-open');
    ...
    self.addClass('ksphere-animate-open');
...
--------------------------------------------------------------*/
$(document).on("click", ".ksphere-animate-btn", function(e){
  e.preventDefault();
  var self    = $(this);
...
function clickToSlide($this){
  ...
  var self    = $this,
    content   = $('#ksphere-details'),
    col   = $('.ksphere-col-content'),
    close     = $('#ksphere-close-content-slide'),
...
$(document).on("click", ".ksphere-content-view", function(e){
  e.preventDefault();
  var self = $(this);
...
--------------------------------------------------------------*/
$(document).on("click", "#ksphere-close-content", function(e){

  e.preventDefault();

  var self    = $(this),
    details   = $('#ksphere-details'),
    btn       = $('.ksphere-animate-btn'),
    content   = $('#ksphere-animate-area'),
    close     = $('#ksphere-close-content'),
    hide      = content.data('hide'),
    animation = details.data('animation');
    ...
    close.hide();
    btn.removeClass('ksphere-animate-open');
    content.removeClass(hide).addClass(animation).addClass('mrs-active');
    details.removeClass(animation).addClass(hide).removeClass('mrs-content-appear').addClass('mrs-conent-disapear');
  
});

$(document).on("click", "#ksphere-close-content-slide, .ksphere-col-content.mrs-default-content-off", function(e){

  e.preventDefault();

  var self    = $(this),
    details   = $('#ksphere-details'),
    btn       = $('.ksphere-content-view'),
    close     = $('#ksphere-close-content-slide'),
    col   = $('.ksphere-col-content'),
    animation = details.data('animation');
...
if ( $("#ksphere-form").is_exist() ) {

  var mform = $("#ksphere-form");
  mform.ajaxChimp({
      callback: callbackFunction,
      url: 'https://xyz.us14.list-manage.com/subscribe/post?u=3e624ea3457b50d638f1bd58b&id=bc67dbaebe'
  });
  function callbackFunction (resp) {
      if (resp.result === 'success') {
        $('#ksphere-email').val('');
        mform.addClass('mform-success');
        if ( $('.ksphere-newsletter-header').length > 0 ) {
          $('.ksphere-newsletter-header').removeClass('mform-header-animate').addClass('mfrom-header-animate-close');
        }
        setTimeout(function(){
          mform.removeClass('mform-success').removeClass('mform-submitting').removeClass('mform-animate');
          if ( $('.ksphere-newsletter-header').length > 0 ) {
            $('.ksphere-newsletter-header').removeClass('mfrom-header-animate-close').addClass('mfrom-header-animate');
          }
...
$(window).on("load resize",function(){
  ...
  if ( $(".ksphere-main-content").is_exist() ) {

    var content = $(".ksphere-main-content"),
        close   = $('#ksphere-close-content');
...
/*--------------------------------------------------------------
  21. Preloader
--------------------------------------------------------------*/
$(".ksphere-loading-screen").delay(200).fadeOut('slow');
